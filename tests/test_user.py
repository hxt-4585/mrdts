...
class TestUser(unittest.TestCase):
    ...
    def setUp(self):
        self.region = Region(RegionConfig.default(), rng=RandomStreams.from_config().region)
    ...
    def test_generate_from_region_assigns_all_users_to_regions(self):
        ...
        users = User(self.config, rng=RandomStreams.from_config().user)
    ...
    def test_generate_from_region_is_reproducible_with_seed(self):
        ...
        first = User(self.config, rng=RandomStreams.from_config().user)
        second = User(self.config, rng=RandomStreams.from_config().user)
    ...
    def test_center_bias_reduces_mean_distance_to_region_centers(self):
        ...
        uniform_users = User(uniform_config, rng=RandomStreams.from_config().user)
        centered_users = User(centered_config, rng=RandomStreams.from_config().user)
    ...
    def test_generate_from_region_requires_region_map(self):
        ...
        with self.assertRaises(RuntimeError):
            User(self.config, rng=RandomStreams.from_config().user).generate_from_region(Region(RegionConfig.default(), rng=RandomStreams.from_config().region))
    ...
    def test_num_users_before_generation_names_current_entry_point(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "generate_from_region"):
            _ = User(self.config, rng=RandomStreams.from_config().user).num_users
