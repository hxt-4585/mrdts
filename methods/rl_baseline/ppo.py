"""Small CPU PPO implementation with immutable rollout observations and masks."""
...
class PPO:
    ...
    def update(self, rollout):
        ...
        state = torch.as_tensor(rollout.critic_observations, dtype=torch.float32)
        returns = torch.as_tensor(rollout.returns, dtype=torch.float32)
        advantages = returns - torch.as_tensor(rollout.values, dtype=torch.float32)
        ...
        for _ in range(cfg.epochs):
            for indices in torch.randperm(len(actions)).split(cfg.minibatch_size):
