...
当前实现已删除实体侧的 `MemberUAV.core_available_at`；`SchedulingRuntime` 内的 `ServerState` 是时隙内核心占用的唯一状态源。

## 决定

- 每个时隙创建一个全新的 `SchedulingRuntime`。
- 信道队列、服务器队列、核心占用和 DAG 运行时仅在当前时隙有效。
- 时隙调度窗口结束时，运行时将未完成 DAG 记为失败，再丢弃整个 `SchedulingRuntime`；未完成工作不带入下一时隙。
...
- 当前不启用通信覆盖半径约束；用户关联采用“同区域最近 Member”，本区域 Member 执行候选也不按覆盖半径筛选。`coverage_radius` 配置及其字段暂时保留，覆盖半径检查不属于当前模型要求。越界动作拒绝和逐 Member 违规标记已实现；具体奖励惩罚数值与空区域处理仍由后续强化学习环境定义。
